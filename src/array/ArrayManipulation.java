...
public class ArrayManipulation {
	...
	public static void main(String[] args) {
		...
		// Allocate the memory to the array dynamically as n can be of order 10^7 and
		// use long as values after summing may exceed range of int
		long[] arr = new long[n + 2]; // Keep the size n+2 as we'll be using 1-indexed array and we're also accessing
										// (b+1)th element
		...
		long res = 0; // res will store the answer to the problem, i.e., maximum value present in the
						// array after all the manipulations

		// This is the implementation of prefix sum array (or cumulative array)
		// Iterating through all the elements, we'll keep adding the elements left to
		// right
		for (int i = 1; i <= n; i++) {
			arr[i] += arr[i - 1];
